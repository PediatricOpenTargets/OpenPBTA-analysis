...
This notebook updates the current [manuscript metadata](https://github.com/rokitalab/OpenPedCan-manuscript/blob/main/content/metadata.yaml) such that the authors are ordered in the following way:

* The first 4 and last 2 authors, are pinned to reflect scholarship and contributor roles that are not captured in the Git history of this repository (e.g., substantive analytical code review, conceptualization, supervision, or project administration).
...
```{r}
manuscript_metadata_url <- "https://raw.githubusercontent.com/rokitalab/OpenPedCan-manuscript/main/content/metadata.yaml"
...
```{r}
pinned_first_authors <- c("Zhuangzhuang Geng", 
                    "Eric Wafula",
                    "Ryan J. Corbett",
                    "Yuanchao Zhang")

pinned_first_positions <- c(1:4)
names(pinned_first_positions) <- pinned_first_authors


pinned_last_authors <- c("Deanne Taylor",
                         "Jo Lynne Rokita")

pinned_authors <- c(pinned_first_authors, pinned_last_authors)

# The indices of the pinned author positions
pinned_last_positions <- str_which(author_names, 
                              paste(pinned_last_authors, collapse = "|"))
# (Author) named vector of indices
names(pinned_last_positions) <- author_names[pinned_last_positions] 

pinned_positions <- c(pinned_first_positions, pinned_last_positions)
...
code_contributors <- module_contributions_df %>%
  ...
  filter(!author %in% pinned_authors) %>%
  pull(author)
...
code_contributor_positions <- 1:length(code_contributors) + 4
...
```{r}
# manually reset last author positions in case of authors missing from yaml file:
pinned_last_positions <- c(max(noncode_contributor_positions)+1, max(noncode_contributor_positions)+2)
names(pinned_last_positions) <- c("Deanne Taylor", "Jo Lynne Rokita")
